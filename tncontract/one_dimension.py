...
def tensor_to_mps(tensor, phys_labels=None, mps_phys_label='phys',
    ...
    left_label : str
        ...
        of the resulting MPS. Also used as `left_label` for the resulting MPS.
    right_label : str
        Label for index of `tensor` that will be regarded as the rightmost
        index of the resulting MPS. Also used as `right_label` for the
        resulting MPS.
    """
    ...
    for k in range(nsites-1):
        U, S, V = tensor_svd(t, [left_label, phys_labels[k]])
...
def tensor_to_mpo(tensor, physout_labels, physin_labels,
    ...
    left_label : str
        Label for index of `tensor` that will be regarded as the leftmost index
        of the resulting MPO. Also used as `left_label` for the resulting MPO.
    right_label : str
        Label for index of `tensor` that will be regarded as the rightmost
        index of the resulting MPO. Also used as `right_label` for the
        resulting MPO.
    """
    ...
    mpo = []
    for k in range(nsites-1):
        U, S, V = tensor_svd(t, [left_label, physout_labels[k],
            physin_labels[k]])
        U.replace_label('svd_in', right_label)
